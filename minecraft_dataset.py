...
class MinecraftSchematicDataset(Dataset):
    ...
    def _scan_files(self):
        ...
        for file_path in tqdm(self.schematic_files):
            try:
                ...
                if height < self.min_dimension or length < self.min_dimension or width < self.min_dimension:
                    continue
    ...
    def _extract_chunk(self, blocks, start_y, start_z, start_x):
        ...
        chunk = np.full((chunk_size, chunk_size, chunk_size), self.block_to_idx['<pad>'], dtype=np.int64)
        ...
        for y in range(chunk_height):
            for z in range(chunk_length):
                for x in range(chunk_width):
                    ...
                    chunk[y, z, x] = self.block_to_idx.get(block_name, self.block_to_idx['<unk>'])
