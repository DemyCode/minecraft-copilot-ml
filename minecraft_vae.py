...
class MinecraftVAE(nn.Module):
    ...
    def __init__(self, 
        ...
        self.flatten_size = hidden_dims[-1] * (chunk_size // (2 ** len(hidden_dims))) ** 3
        ...
        decoder_layers.append(
            nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.Conv3d(hidden_dims_reversed[-1], embedding_dim, kernel_size=3, padding=1),
            )
        )
    ...
    def interpolate(self, x1, x2, mask1=None, mask2=None, steps=10, device="cuda"):
        ...
        """
        # Encode both inputs
        mu1, log_var1 = self.encode(x1.unsqueeze(0), mask1.unsqueeze(0) if mask1 is not None else None)
        mu2, log_var2 = self.encode(x2.unsqueeze(0), mask2.unsqueeze(0) if mask2 is not None else None)
